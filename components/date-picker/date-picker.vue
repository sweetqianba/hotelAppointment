		...
		<view class="calendar-layer" :animation="animationData" :class="isShow_H5 ? 'show' : 'hide'">
			...
			<view class="layer-content" :class="{ choiceDate: choice === true }">
								...
								<view v-for="(data, index2) in monthData" :key="index2" class="day" :data-index="index" :data-indexs="index2"
								 :class="
										data.dateTime < today
											...
											: ''
									"
								 :style="index2 == 0 ? 'margin-left:' + (weeks[index] == 0 ? 0 : weeks[index]) * (100 / 7) + '%' : ''" @tap="dayClick">
				...
				<view class="layer-footer">
					<view class="submitBtn" @tap="submitbtn" v-if="choice === true">完成</view>
				</view>
			</view>
		</view>
	</view>
	...
	export default {
		...
		methods: {
			...
			dateData: function() {
				...
				let startIndex2 = start_year == year && start_month == month ? start_day - day : start_day - 1;

				//当前选中的结束时间坐标
				let endIndex1 = end_year == year ? end_month - month : end_month + (12 - month);
				let endIndex2 = end_year == year && end_month == month ? end_day - day : end_day - 1;
				//
			...
			getWeek(weeks, firstMonth, curMonth, day) {
				...
				let firstDayWeek = weeks[curMonth - firstMonth];
			...
			selectday: function(index, indexs) {
				if (indexs == -1) {
					return;
				}
